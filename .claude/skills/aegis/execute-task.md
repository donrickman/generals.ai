...
The test: **would a skilled contractor call the client for this?** If yes, surface a challenge via `mcp__aegis__raise_challenge`. If no, handle it and keep going. At the end, always call `mcp__aegis__report_result` — a task with no report_result is treated as a failure.
...
3. Call `mcp__aegis__report_result(status="succeeded", summary="...")`
...
- Call `mcp__aegis__report_progress` every 3 minutes and at every meaningful milestone
- Call immediately when you discover something significant
- Raise a challenge immediately when blocked — don't sit on it
- Never go silent for > 5 minutes
- Call `mcp__aegis__report_result` at the end of every task — no exceptions
...
| CAPTCHA / login wall | Switch to browser automation via Playwright MCP tools |
| Same error 3x | Change approach entirely — stop guessing |
...
**Browser reads:** Call `mcp__playwright__browser_snapshot()` to read any page. Never dump raw HTML into context.

## Completion Gate

Before calling `mcp__aegis__report_result`:
